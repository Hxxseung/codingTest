...
public class BeakJoon15650 {
    ...
    public static void main(String[] args) {
        ...
        tracking(N, M, 0, 0); // 백트래킹 시작
    }

    private static void tracking(int N, int M, int depth, int start) {
        ...
        for (int i = start + 1; i <= N; i++) { // start + 1부터 시작
            if (!visited[i]) { // 방문하지 않은 숫자만 선택
                visited[i] = true; // 방문 표시
                result[depth] = i; // 현재 숫자를 결과 배열에 저장
                tracking(N, M, depth + 1, i); // 다음 깊이로 재귀 호출
